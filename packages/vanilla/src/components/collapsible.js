...
//
// Animation pattern: the CSS owns the transition (via the GLOBAL
// [data-collapsing] rule on the animated element); JS toggles the attribute
// around a reflow-bracketed height change. Putting the transition spec in CSS
// — instead of writing inline transition values from JS — sidesteps the
// foot-gun where same-task transition + property changes can fall through to
// the previous (none) transition computation in some browsers.
//
// Anatomy:
...
const COLLAPSING_ATTR = 'data-collapsing';
...
export class Collapsible extends Component {
  ...
  async open() {
    ...
    this._awaitTransition(this._duration(), () => {
      if (!this.el) return;
      this.el.removeAttribute(COLLAPSING_ATTR);
  ...
  async close() {
    ...
    // 2. Add [data-collapsing] so the CSS transition rule applies.
