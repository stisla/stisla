...
//   3. [title] attr — migrated and stripped so native tooltip doesn't flash. Restored on destroy().
...
export class Tooltip extends Component {
  ...
  async _position() {
    ...
    const { x, y, placement, middlewareData } = await computePosition(
      ...
      this._tooltipEl,
      {
        // 'fixed' so any ancestor with overflow: hidden can't crop the tooltip. CSS matches.
        strategy: 'fixed',
    ...
    // Position the arrow along the cross-axis. arrow() returns x for top/bottom placements and y for
    // left/right (FUI's standard shape).
...
// No module-level delegated handler — tooltip listeners are wired per-instance on the trigger
// (mouseenter/mouseleave/focusin/focusout). HMR is safe because the Component base re-init path
// destroys the old instance (detaching its recorded listeners) before the new one binds.
