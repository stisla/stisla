...
// Scroll lock + shake are attribute hooks (CSS reads html[data-drawer-open] and
// .drawer__content[data-shaking]); recast from the legacy is-* classes per the no-is-* convention.
const SCROLL_LOCK_ATTR = 'data-drawer-open';
...
export class Drawer extends Component {
  ...
  shake() {
    ...
    this._content.removeAttribute(SHAKING_ATTR);
    void this._content.offsetWidth;
    this._content.setAttribute(SHAKING_ATTR, '');
    setTimeout(() => this._content?.removeAttribute(SHAKING_ATTR), 250);
  }
...
if (typeof document !== 'undefined' && typeof window !== 'undefined' && !window.__stislaDrawerBound) {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (dismiss) {
      ...
      // Explicit dismiss controls (.drawer__close, footer buttons) always close.
