...
//
// Markup-first: popover content lives in the DOM as real children (titles, links, buttons, forms).
// The imperative path can rewrite content via setContent({ title, body }).
...
export class Popover extends Component {
  ...
  constructor(el, opts) {
    ...
    // class injects one so FUI's arrow() middleware has a target.
  ...
  open() {
    ...
    // the popover. Original DOM position is restored on close.
    ...
    // against a measurable element. Without this FUI reads 0×0 → NaN.
    ...
    requestAnimationFrame(() => {
      ...
      // Focus trap only for click/manual triggers. Hover-trigger popovers stay
      // read-only — moving the trap would yank focus from the trigger.
  ...
  close({ returnFocus } = {}) {
    ...
    waitForTransition(this.el).then(() => {
      ...
      // Restore original DOM position so the author's tree is intact for re-open.
  ...
  destroy() {
    ...
    if (this.el?.dataset.state === OPEN) {
      ...
      document.removeEventListener('keydown', this._onDocKeydown, true);
      // If destroy() lands mid-open, restore DOM position.
  ...
  async _position() {
    ...
    const { x, y, placement, middlewareData } = await computePosition(
      ...
      this.el,
      {
        // 'absolute' strategy with the popover portaled to <body>: x/y are
        // document coordinates, so scrolling moves popover + trigger together.
        strategy: 'absolute',
...
if (
  ...
  document.addEventListener('click', (e) => {
    // Trigger click → toggle the matched popover. Per-attr opts re-read on every toggle.
    ...
    if (opener) {
      ...
      if (surface && surface.classList.contains('popover')) {
        ...
        // Skip click toggling for hover/focus triggers — clicking should not fight the hover state.
        const triggers = String(inst.opts.triggerMode).split(/\s+/).filter(Boolean);
        if (triggers.includes('click')) {
          // One popover open at a time: close any other open popover before toggling this one.
          // Leaves pinned (autoClose:false) and hover/focus popovers alone.
    ...
    // Auto-close pass for every open popover. Hover/focus triggers skip this path.
