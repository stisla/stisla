...
//     …
//
// State hooks are data-attributes (no is-* per the v3 convention):
//   keyboard cursor → [data-highlighted]; selected row → [aria-selected="true"].
//
...
export class Select extends Component {
  ...
  _renderOptions(scopeId) {
    ...
    const mkOption = (opt) => {
      ...
      // Leading check indicator (shown via [aria-selected="true"]; the slot is
      // always reserved for column alignment). aria-hidden — aria-selected on the
      // row carries the state for assistive tech.
