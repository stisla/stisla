...
// starter and packs it into dist/meridian-dashboard.zip.
//
// What ships in the zip (a single `dashboard/` folder, because every nav link is
// absolute `/dashboard/<page>.html` — drop the folder at your web root and the
// links resolve):
...
//   assets/css/dashboard/   @source is repointed at the rendered *.html)
...
const OUT = path.join(DIST, "dashboard"); // mounts at /dashboard/
const ZIP = "meridian-dashboard.zip";
...
//    rendered HTML (the zip has no .njk): pages sit at dashboard/*.html, app.css
//    at dashboard/assets/css/app.css, so "../../*.html".
...
await fs.cp(rel("assets/css/dashboard"), out("assets/css/dashboard"), { recursive: true });
...
//    dist/dashboard/ for inspection.
execFileSync("zip", ["-r", "-q", ZIP, "dashboard"], { cwd: DIST });
console.log(`\npacked  dist/${ZIP}`);
