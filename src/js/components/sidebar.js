...
//
// Also owns standalone rail collapse — flipping .is-collapsed on the
// panel in response to [data-stisla-sidebar-toggle="collapse"] triggers
// (delegated, document-level). For sidebars inside an .app-shell, the
// shell's data-app-shell-toggle="collapse" already handles this; the
// sidebar-level handler covers the no-shell case and exposes a
// programmatic collapse()/expand()/toggleCollapsed() API.
//
// Anatomy:
...
//
// Events (bubbling):
//   stisla:sidebar:submenu-change   — detail: { sidebar, item, open }
//                                     after a submenu's transition settles
//   stisla:sidebar:collapse-change  — detail: { sidebar, collapsed }
//                                     after .is-collapsed flips
//
// Submenu events source from bubbling stisla:collapsible:opened/closed so
// user-clicks and programmatic calls funnel through the same point —
// same shape as Accordion. Each submenu's Collapsible still emits
// stisla:collapsible:*.
...
export class Sidebar extends Component {
  ...
  constructor(el, opts) {
    ...
    // Seed any pre-existing collapse triggers' aria-expanded so they
    // match the panel's initial .is-collapsed state. The data-attr
    // semantic is "expanded?", matching aria-expanded's natural reading.
  ...
  _setCollapsed(next) {
    ...
    // Close any open submenus before flipping .is-collapsed so each
    // Collapsible's height transition runs alongside the panel's width
    // transition (instead of snapping shut on the rail-mode
    // display: none). The CSS carve-out for .is-collapsing keeps the
    // submenu rendered through its close animation; the existing
    // [data-state="closed"] rule takes over once the transition settles.
    // Expand intentionally does NOT re-open previously-open submenus —
    // the user clicks back in to whichever menu they want.
  ...
  // Walks for matching collapse triggers — inside the sidebar plus any
  // external buttons that target it via aria-controls — and updates
  // aria-expanded so triggers stay coherent regardless of which one
  // fired the state change (or whether it came from the programmatic API).
...
if (typeof document !== 'undefined' && typeof window !== 'undefined' && !window.__stislaSidebarBound) {
  ...
  document.addEventListener('click', (e) => {
    ...
    // anywhere in the page); otherwise climb to the nearest sidebar
    // ancestor. Skips silently when neither resolves.
    ...
    // Belt-and-braces: if Stisla.init() hasn't reached this panel yet
