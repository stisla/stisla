...
// is native — the browser owns checkbox/radio state, the sibling
// combinator owns paint, no JS instance attaches.
//
...
//
// Opts:
//   pressed: null | boolean    null = read initial aria-pressed; explicit
//                              boolean writes it on init.
...
export class Toggle extends Component {
  ...
  constructor(el, opts) {
    ...
    // Initial state. An explicit opt wins; otherwise read what markup
    // declared. Default to false if neither path supplies a value so the
    // attribute is always set on the host post-init.
    let initial;
