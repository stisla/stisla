...
// declarative data-attr API. The skin lives in the matching SCSS partial
...
// component-layer rules (`@layer components` in _scroll-area.scss) win
...
function _injectOverlayScrollbarsCss() {
  ...
  style.textContent = `@layer foundation { ${overlayScrollbarsCss} }`;
...
// present (CDN / zip use). Mirrors the carousel pattern: the core's
// microtask init() has already fired, so we also re-init to catch any
// pre-existing [data-stisla-scroll-area] on the page.
