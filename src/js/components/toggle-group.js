...
//
// Anatomy:
//   <div class="toggle-group" data-stisla-toggle-group
//        role="radiogroup|group" aria-label="…">
...
//
// Type:
//   'single'   — radio semantics; one (or zero) member active. Auto-select
//                on arrow per WAI-ARIA radio-group.
//   'multiple' — independent press toggles. Arrows move focus only; Space/
//                Enter commits.
//
// Type autodetect when opts.type is unset:
//   role="radiogroup" on the wrapper OR any role="radio" member → 'single'
//   otherwise                                                   → 'multiple'
//
// Orientation autodetect when opts.orientation is unset:
//   .toggle-group--vertical                                     → 'vertical'
//   otherwise                                                   → 'horizontal'
//
// Form-data path guard:
//   If the wrapper contains a child .toggle-input (form-data path), the
//   browser already owns selection. Bail with a dev warning — attaching
//   would fight native.
//
// Events (bubbling, detail: { group: this, member, value }):
//   stisla:toggle-group:change  — fired post-flip
//     single   detail: { value, member, previousValue, previousMember }
//     multiple detail: { value: string[], member, action: 'pressed'|'unpressed' }
//
// Opts:
//   type:         null | 'single' | 'multiple'      (null → autodetect)
//   orientation:  null | 'horizontal' | 'vertical'  (null → autodetect)
//   loop:         true                              (wrap arrow nav at ends)
//   rovingFocus:  true                              (one tabbable member at a time)
...
export class ToggleGroup extends Component {
  ...
  constructor(el, opts) {
    ...
    // String coercion: per-attr parser hands JSON-tried strings through,
    // so 'true'/'false' arrive as strings when JSON.parse rejects them.
    // (true/false parse fine; only safety net for non-JSON values.)
  ...
  _onKeydown(e) {
    ...
    switch (e.key) {
      ...
      case ' ':
      case 'Enter': {
        // Native <button> dispatches click for Space/Enter. Single mode
        // already auto-selected on arrow focus, so Space/Enter is a no-op
        // refresh — let the click bubble through naturally rather than
        // double-firing here. Multiple mode wants Space/Enter to commit;
        // again the native click handles it. No preventDefault — keeps
        // form behavior consistent and avoids fighting the browser.
        return;
      }
    }
  }
  ...
  _onFocusin(e) {
    ...
    // so Tab leaves the group naturally on next press. For 'single' mode
    // the active member is the canonical tab stop; we only re-rove when
    // the user explicitly focuses a different member.
    const member = e.target.closest('.toggle');
    if (!member || this._getMembers().indexOf(member) === -1) return;
  ...
  _focusSibling(from, delta) {
    ...
    // Disabled `from` (focused via JS edge case) — treat the cursor as
    // sitting just past the appropriate end so the delta lands on the
  ...
  _initRovingTabindex() {
    ...
    // For single mode the active member is the canonical tab stop. Falls
    // back to the first enabled member if no member is active yet.
    // For multiple mode the first enabled member is the tab stop until
    // the user focuses a specific one.
    let initial;
    if (this.opts.type === SINGLE) {
      initial =
