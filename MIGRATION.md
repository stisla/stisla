...
- [ ] Step 3 — Component rewrite (per §4 table, in order from §3 step 3)
  ...
  - [ ] **🚧 Height amendment — controls use `height` not `min-height`** (architecture fix; touches `.btn` + `.form-control` + density doc; **do BEFORE 3.3b** so input-group sizing inherits — see Status line at top of file for the full rationale)
  - [ ] 3.3b — form-check + input-group
  - [ ] 3.3c — form-range
- [ ] Step 4 — JS rewrite (interleaved with step 3 JS-coordinated components)
