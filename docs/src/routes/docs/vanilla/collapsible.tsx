  ...
  return (
    <>
      ...
      <section>
        <h2>Basic</h2>
        <p>
          The primitive that <code>.accordion</code>, <code>.sidebar</code> submenus, and{" "}
          <code>.navbar</code> mobile menu compose for their own collapse behavior. A trigger button
          and a content region tied by id. The trigger points at the region via{" "}
          <code>data-stisla-collapsible-trigger</code>; the region carries{" "}
          <code>data-stisla-collapsible</code> and <code>data-state="open"</code> or{" "}
          <code>"closed"</code> driving the paint. The height transition runs around every state
          flip.
        </p>
        <Demo
          layout="stack"
...
<div class="flex flex-col items-center gap-3">
  ...
  <div id="collapsible-basic" class="collapsible" data-stisla-collapsible data-state="closed" role="region">
    <div class="rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] p-4">A collapsible is a region whose visibility flips between open and closed. The state lives on the element itself; the height transition runs around the flip so the panel does not jump.</div>
  </div>
</div>`}
        />
      </section>
      ...
      <section>
        <h2>Keyboard</h2>
        <p>
          The trigger is whatever element you wire to the region. The collapsible itself adds no key
          bindings beyond the trigger's native ones. When the trigger is a native{" "}
          <code>&lt;button&gt;</code>, the browser handles focus and activation.
        </p>
        <ul>
          <li>
      ...
      <section>
        ...
        <p>
          Render the region with <code>data-state="open"</code> to start open. The trigger's{" "}
          <code>aria-expanded</code> follows on construction.
        </p>
        <Demo
          layout="stack"
          html={`
<div class="flex flex-col items-center gap-3">
  ...
  <div id="collapsible-open" class="collapsible" data-stisla-collapsible data-state="open" role="region">
    <div class="rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] p-4">This region started open. Click the trigger to close it. The opening transition is the same one the trigger fires.</div>
  </div>
</div>`}
        />
      </section>
      ...
      <section>
        <h2>Inside a card</h2>
        <p>
          A common shape: a card with extra detail hidden behind a trigger. The region's content owns
          its own surface so the card body doesn't paint inside the collapsed footprint.
        </p>
        <Demo
          layout="stack"
          html={`
  ...
  <div class="card__body">
    ...
    <div id="collapsible-card" class="collapsible" data-stisla-collapsible data-state="closed" role="region">
      ...
      <section>
        ...
        <p>
          Build a <code>Stisla.Collapsible</code> directly to drive a region from code. The buttons
          below call <code>open()</code>, <code>close()</code>, and <code>toggle()</code> on an
          instance from <code>Stisla.Collapsible.getOrCreate(el)</code>.
        </p>
        <Demo
          layout="stack"
          html={`
<div class="flex flex-col items-center gap-3">
  ...
  <div id="collapsible-programmatic" class="collapsible" data-stisla-collapsible data-state="closed" role="region">
    <div class="rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] p-4">This region is opened and closed by the buttons above through the imperative API.</div>
  </div>
</div>
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (e) {
      ...
      <section>
        <h2>Events</h2>
        <p>
          Four events bubble from the region element. The <code>opening</code> and{" "}
          <code>closing</code> events are cancelable.
        </p>
        <p>
          <code>stisla:collapsible:opening</code> fires before the open transition starts. Call{" "}
          <code>preventDefault()</code> to abort.
        </p>
        <p>
          <code>stisla:collapsible:opened</code> fires after the open transition ends.
        </p>
        <p>
          <code>stisla:collapsible:closing</code> fires before the close transition starts. Call{" "}
          <code>preventDefault()</code> to keep it open.
        </p>
        <p>
          <code>stisla:collapsible:closed</code> fires after the close transition ends.
        </p>
      </section>

      <section>
        <h2>Customization</h2>
        <p>One variable retunes the animation. Override on the root, a wrapper, or a single region.</p>
        <table>
          <thead>
            <tr>
              <th>Variable</th>
