...
export const Route = createFileRoute("/docs/vanilla/timeline")({
...
function TimelineDocs() {
  return (
    <>
      <header>
        <h1>Timeline</h1>
        <p className="lead">A sequence of events laid along a rail.</p>
      </header>

      <section>
        <h2>Basic</h2>
        <p>
          Each event is a <code>.timeline__item</code> holding a <code>.timeline__marker</code> on the
          rail and a <code>.timeline__body</code> beside it. The rail line draws itself between markers
          and stops at the last one. It is pure layout with no JS; status rides an author-set{" "}
          <code>data-state</code> and intent rides a <code>.timeline__marker--*</code> modifier. An
          empty marker paints a dot on the rail. Use an <code>&lt;ol&gt;</code> for the root so the
          order carries to screen readers.
        </p>
        <Demo
          layout="stack"
          html={`
<ol class="timeline max-w-lg">
  ...
  <li class="timeline__item">
    ...
    <div class="timeline__body">
      ...
      <section>
        <h2>Icon and avatar markers</h2>
        <p>
          Drop an icon inside a marker for a system event or an avatar for a person. Every marker is the
          same size, so they line up however you mix them; bump <code>--timeline-marker-size</code> on
          the root to scale them together. Avatars and images sit on the rail directly, without the
          tinted ring an icon marker gets.
        </p>
        <Demo
          layout="stack"
          html={`
  ...
  <li class="timeline__item">
    ...
    <div class="timeline__body">
      ...
      <section>
        <h2>Status</h2>
        <p>
          Set <code>data-state</code> on an item to show where it sits in a process. A{" "}
          <code>complete</code> step fills its marker and keeps the rail solid, <code>current</code>{" "}
          rings the marker and dashes the rail leaving it, and <code>pending</code> mutes the marker and
          dashes the rest.
        </p>
        <Demo
          layout="stack"
...
<ol class="timeline max-w-lg">
  ...
  <li class="timeline__item" data-state="pending">
    ...
    <div class="timeline__body">
      ...
      <section>
        <h2>Time at the bottom</h2>
        <p>
          The body is a plain stack, so the time sits wherever you place it. Lead with the title and
          drop <code>.timeline__time</code> to the last line when it reads better as a caption. The
          marker aligns to the first line.
        </p>
        <Demo
          layout="stack"
          html={`
  ...
  <li class="timeline__item">
    ...
    <div class="timeline__body">
      ...
      <section>
        <h2>Marker colors</h2>
        <p>
          Add a <code>.timeline__marker--*</code> modifier to color a marker for a fixed meaning. Color
          is separate from status, so a marker can carry an intent color whether or not the item has a{" "}
          <code>data-state</code>. For a one-off color, set <code>--timeline-marker-color</code> on the
          item instead.
        </p>
        <Demo
          layout="stack"
  ...
  <li class="timeline__item">
    ...
    <div class="timeline__body">
      ...
      <section>
        <h2>Alternate</h2>
        <p>
          Add <code>.timeline--alternate</code> to center the rail and place events on either side of
          it. It needs room, so on a narrow screen it folds back to the standard left rail. Status and
          marker content work the same way.
        </p>
        <Demo
          layout="stack"
          html={`
  ...
  <li class="timeline__item" data-state="pending">
    ...
    <div class="timeline__body">
      ...
      <section>
        <h2>Customization</h2>
        <p>
          These variables retune <code>.timeline</code> without touching component CSS. Set them on the
          timeline, a parent scope, or <code>:root</code>. The marker color also reads on a single{" "}
          <code>.timeline__item</code>, which is how the intent modifiers and a one-off override work.
          For a neutral activity feed, set <code>--timeline-marker-color</code> to{" "}
          <code>var(--color-muted-foreground)</code> on the root.
        </p>
        <h3>Geometry</h3>
        <table>
          <thead>
          ...
          <tbody>
            <tr><td><code>--timeline-marker-size</code></td><td>Marker size; every marker is a circle of this size and one value scales them all</td></tr>
            <tr><td><code>--timeline-dot-size</code></td><td>Diameter of the dot shown when a marker is empty</td></tr>
            <tr><td><code>--timeline-connector-width</code></td><td>Thickness of the rail line</td></tr>
            <tr><td><code>--timeline-connector-gap</code></td><td>Space between a marker and the line at both ends of each segment</td></tr>
            <tr><td><code>--timeline-connector-style</code></td><td>Rail line style; solid by default, the current and pending states switch it to dashed</td></tr>
            <tr><td><code>--timeline-gutter</code></td><td>Space between the rail and the event body</td></tr>
            <tr><td><code>--timeline-gap</code></td><td>Vertical space between events</td></tr>
          </tbody>
        </table>
        <h3>Paint</h3>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>--timeline-connector-color</code></td><td>Rail line color</td></tr>
          </tbody>
        </table>
      </section>
    </>
